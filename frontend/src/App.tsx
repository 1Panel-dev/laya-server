...
function useSession() {
  ...
  useEffect(() => {
    api<Session>("/internal/auth/session")
      .then(setSession)
...
function BrandLogo() {
  return (
    <img
      ...
      src="/laya-server-logo-black.png"
      alt=""
      aria-hidden="true"
    />
  );
      ...
      <div className="main">
        <header className="topbar">
          ...
          <div className="topbar-controls">
            <span className="topbar-note">LAYA SERVER</span>
            <LanguagePicker />
          </div>
        </header>
        <main className="content">
          {page === "home" ? (
            ...
            <Keys csrf={session.csrf_token} />
          ) : page === "usage" ? (
            <UsagePage />
          ) : page === "playground" ? (
            <Playground csrf={session.csrf_token} />
          ) : (
            <Docs />
          )}
        </main>
...
function Home({ setPage }: { setPage: (page: Page) => void }) {
  ...
  useEffect(() => {
    api<ApiKey[]>("/internal/api-keys")
      .then((keys) => setKeyCount(keys.filter((key) => !key.revoked_at).length))
      .catch(() => { });
  }, []);
  return (
    <>
      ...
      <div className="home-intro">
        ...
        <h2>
          {keyCount
            ? t("activeKeyCount", { count: keyCount })
            : t("createFirstKey")}
        </h2>
        <p>{t(keyCount ? "homeWithKeys" : "homeWithoutKeys")}</p>
        <Button onClick={() => setPage("keys")}>{t("manageKeys")}</Button>
      </div>
      <div className="home-links">
        <button onClick={() => setPage("playground")}>
          <Send size={19} />
          <strong>{t("tryPlayground")}</strong>
          <span>{t("tryPlaygroundDescription")}</span>
        </button>
        <button onClick={() => setPage("docs")}>
          <BookOpen size={19} />
          <strong>{t("viewDocs")}</strong>
          <span>{t("viewDocsDescription")}</span>
        </button>
      </div>
    </>
  );
...
function Keys({ csrf }: { csrf: string }) {
  ...
  const [busy, setBusy] = useState(false);
  const reload = () =>
    api<ApiKey[]>("/internal/api-keys").then(setKeys).catch(setError);
  useEffect(() => {
    void reload();
  }, []);
  ...
  async function revoke(key: ApiKey) {
    ...
    try {
      await api(
        "/internal/api-keys/" + key.id + "/revoke",
        { method: "POST" },
        csrf,
      );
      await reload();
    ...
    <>
      ...
      {rawKey ? (
        <div className="key-reveal">
          <div>
            <strong>{t("saveKeyTitle")}</strong>
            <p>{t("saveKeyDescription")}</p>
          </div>
      ...
      <div className="section-head">
        <h2>{t("keyList")}</h2>
        <span>{t("keyCount", { count: keys.length })}</span>
      </div>
      {keys.length === 0 ? (
        <div className="empty">
          <KeyRound size={23} />
          <h3>{t("noKey")}</h3>
          <p>{t("noKeyDescription")}</p>
            ...
            <tbody>
              {keys.map((key) => (
                <tr key={key.id}>
                  ...
                  <td>{formatDate(key.created_at, locale)}</td>
                  <td>
                    {key.last_used_at
                  ...
                  <td>
                    <span
                      className={key.revoked_at ? "status revoked" : "status"}
                    >
                      {t(key.revoked_at ? "revoked" : "active")}
                    </span>
                  </td>
                  <td>
                    {!key.revoked_at ? (
      ...
      {showCreate ? (
        ...
        >
          ...
          >
            ...
            <h2 id="create-title">{t("createKeyTitle")}</h2>
            <p>{t("createKeyDescription")}</p>
...
function UsagePage() {
  ...
  const [error, setError] = useState<unknown>(null);
  useEffect(() => {
    api<Usage>("/internal/usage").then(setUsage).catch(setError);
  }, []);
  const totals = usage?.totals;
  const daily = usage?.daily || [];
  const maxTokens = Math.max(
    1,
    ...daily.map((day) => day.input_tokens + day.output_tokens),
  );
    ...
    <>
      ...
      <section className="chart-section">
        <div className="chart-head">
          <h2>{t("tokens")}</h2>
          <strong>
            {number((totals?.input_tokens || 0) + (totals?.output_tokens || 0))}
          </strong>
        </div>
        <div className="bars">
          {daily.length ? (
            daily.map((day) => (
              <div
                ...
                title={t("tokenTooltip", {
                  day: day.day,
                  count: number(day.input_tokens + day.output_tokens),
                })}
              >
                <div
                  className="bar"
                  style={{
                    height:
                      Math.max(
                        2,
                        ((day.input_tokens + day.output_tokens) / maxTokens) *
                        100,
                      ) + "%",
                  }}
                />
            ...
            <div className="chart-empty">{t("noUsage")}</div>
          )}
        </div>
      </section>
      <section className="chart-section">
        <div className="chart-head">
          <h2>{t("requests")}</h2>
          <strong>{number(totals?.requests || 0)}</strong>
        </div>
        <div className="bars">
          {daily.length ? (
            daily.map((day) => (
              <div
                className="bar-column"
                key={day.day}
                title={t("requestTooltip", {
                  day: day.day,
                  count: number(day.requests),
                })}
              >
                <div
                  className="bar"
                  style={{
                    height:
                      Math.max(2, (day.requests / maxRequests) * 100) + "%",
                  }}
                />
                <span>{day.day.slice(5)}</span>
              </div>
            ...
            <div className="chart-empty">{t("noRequests")}</div>
          )}
        </div>
      </section>
      <div className="section-head">
        ...
        <table>
          ...
          <tbody>
            {usage?.sources.map((source) => (
              <tr key={source.source + "-" + source.key_id}>
                <td className="strong">
                  {source.source === "playground"
                    ? t("playgroundSource")
                    : "API Key #" + source.key_id}
                </td>
                <td>{number(source.requests)}</td>
                <td>{number(source.input_tokens)}</td>
                <td>{number(source.output_tokens)}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  questions: {
    intent: {
      ...
      criteria: {
        ...
        technical_help: "a technical problem",
      },
    },
    urgent: {
      type: "noul",
...
const chineseSampleRequest = {
  ...sampleRequest,
  state: { message: "我的账户被重复扣费了，请尽快退款。" },
  model: "multilingual",
};
const traditionalSampleRequest = {
  ...sampleRequest,
  state: { message: "我的帳戶被重複扣款了，請盡快退款。" },
  model: "multilingual",
};
        ...
        <section>
          <div className="section-head">
            ...
            <div className="sample-actions">
              <button
                onClick={() => setBody(JSON.stringify(sampleRequest, null, 2))}
              >
                {t("englishExample")}
              </button>
              <button
                onClick={() =>
                  setBody(
                    JSON.stringify(
                      locale === "zh-TW"
                        ? traditionalSampleRequest
                        : chineseSampleRequest,
                      null,
                      2,
                    ),
                  )
          ...
          <Button onClick={submit} disabled={busy}>
            <Send size={16} />{" "}
            {busy ? t("runningInference") : t("runInference")}
          </Button>
        </section>
        <section>
          <div className="section-head">
...
function Docs() {
  ...
  return (
    <>
      ...
      />
      <div className="docs">
        <h2>POST /v1/systemone</h2>
        <p>{t("docsIntro")}</p>
        <pre>{JSON.stringify(sampleRequest, null, 2)}</pre>
        <h2>{t("docsResponse")}</h2>
        <p>{t("docsResponseDescription")}</p>
