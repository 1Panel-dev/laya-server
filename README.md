...
自动选择语言时，按浏览器偏好顺序匹配有效的中文或英文语言标签，跳过格式无效或不支持的语言，均不匹配时使用英文。中文的 `Hans` / `Hant` 字形优先于地区；未指定字形时，台湾、香港和澳门使用繁体，其余使用简体。已有的手动选择仍优先于浏览器偏好。

可使用与 Docker 构建阶段一致的 Node.js 24，在 `frontend/` 目录运行 `pnpm test` 验证语言匹配规则，再运行 `pnpm build` 检查类型和生产构建。
