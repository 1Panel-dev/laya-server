...
请求中的 `state` 可为字符串、JSON 对象或数组；`questions` 是非空问题 ID 映射，支持 `noul`、`choice` 和 `score`；`model` 可选 `auto`（默认）或 `multilingual`。发布镜像只内置 multilingual；显式请求其他模型返回 422。返回保留上游 `answers`、`model` 和 `usage`。错误使用 `detail.code` 和 `detail.message`。无效密钥为 401，校验失败为 422，模型不可用为 503。控制台 Playground 使用管理员会话，记录为单独用量来源。本地源码开发默认仍可使用全部三个模型，前提是已下载对应权重。
